...
export default function PricingPage() {
  ...
  const displayPrice = useMemo(() => (interval === 'monthly' ? displayMonthly : displayYearly), [interval, displayMonthly, displayYearly])
  ...
  fetch('/api/stripe/subscription')
    ...
    .catch(async (err: Response | any) => {
      if (err?.status && (err.status === 401 || err.status === 404)) { setSub(null); return }
  ...
  async function checkout(i: IntervalKey) {
    ...
    try {
      ...
      setLoading(null)
      if (!r.ok || !j?.url) {
        if (r.status === 401) { await signIn(undefined, { callbackUrl: '/pricing' }); return }
  ...
  const sessionBadge = sessionEmail ? `Sessão: ${sessionEmail} • ${isPRO ? 'PRO' : 'FREE'}` : 'Sessão: visitante • FREE'
      ...
      <main className="min-h-[100dvh] bg-app">
        <section className="mx-auto max-w-6xl px-4 sm:px-6 py-10 sm:py-14">
          <header className="text-center">
            <h1 className="text-[clamp(28px,6vw,44px)] font-semibold tracking-tight">Ganhe 1–2 horas por dia no consultório e plantão</h1>
            <p className="mt-2 text-[15.5px] text-muted">Copiloto clínico com evidências. Relatórios e Prescrições prontos em minutos.</p>

            <div className="mt-6 inline-flex items-center rounded-full border border-base bg-panel p-1">
              <button className={`h-10 px-4 rounded-full ${interval === 'monthly' ? 'bg-app shadow-soft' : ''}`} onClick={() => setInterval('monthly')} aria-pressed={interval === 'monthly'}>Mensal</button>
              <button className={`h-10 px-4 rounded-full ${interval === 'yearly' ? 'bg-app shadow-soft' : ''}`} onClick={() => setInterval('yearly')} aria-pressed={interval === 'yearly'}>Anual</button>
            </div>

            <p className="mt-3 text-[13.5px] text-muted">{sessionBadge}</p>
          </header>

          {error && <div className="mx-auto mt-5 max-w-xl rounded-2xl border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
          ...
          <ul className="mt-10 grid gap-5 md:grid-cols-2">
            <li className="rounded-2xl border border-base bg-panel p-5 sm:p-6 flex flex-col">
              ...
              <ul className="mt-4 space-y-2 text-[14px]">
                <li className="flex items-start gap-2"><span className="mt-1 inline-block h-2 w-2 rounded-full bg-emerald-500" aria-hidden /><span>Modo <strong>Geral</strong> ilimitado</span></li>
                <li className="flex items-start gap-2"><span className="mt-1 inline-block h-2 w-2 rounded-full bg-emerald-500" aria-hidden /><span>Modo <strong>Estudos</strong> ilimitado</span></li>
                <li className="flex items-start gap-2"><span className="mt-1 inline-block h-2 w-2 rounded-full bg-emerald-500" aria-hidden /><span>Relatórios básicos e exportação</span></li>
              </ul>
              <div className="mt-6">
                <a href="/dashboard" className="btn rounded-full h-11 px-6 w-full shadow-soft inline-grid place-items-center">Usar grátis agora</a>
              </div>
            </li>
            ...
            <li className="rounded-2xl border border-base bg-panel p-5 sm:p-6 flex flex-col relative overflow-hidden">
              ...
              <ul className="mt-4 space-y-2 text-[14px]">
                <li className="flex items-start gap-2"><span className="mt-1 inline-block h-2 w-2 rounded-full bg-emerald-500" aria-hidden /><span>Plantão, Consultório, Especialidades, Análise+Prescrição</span></li>
                <li className="flex items-start gap-2"><span className="mt-1 inline-block h-2 w-2 rounded-full bg-emerald-500" aria-hidden /><span>Resumos de diretrizes + links de evidência</span></li>
                <li className="flex items-start gap-2"><span className="mt-1 inline-block h-2 w-2 rounded-full bg-emerald-500" aria-hidden /><span>LGPD • Voz 60 min • Suporte prioritário</span></li>
              </ul>

              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                <button onClick={() => checkout('monthly')} disabled={loading === 'checkout-monthly' || isPRO} className="btn rounded-full h-11 px-6 w-full shadow-soft disabled:opacity-60">
                  {isPRO ? 'Você já é PRO' : (loading === 'checkout-monthly' ? 'Abrindo…' : 'Assinar Mensal')}
                </button>
                <button onClick={() => checkout('yearly')} disabled={loading === 'checkout-yearly' || isPRO} className="btn rounded-full h-11 px-6 w-full shadow-soft disabled:opacity-60">
                  {isPRO ? 'Você já é PRO' : (loading === 'checkout-yearly' ? 'Abrindo…' : 'Assinar Anual')}
                </button>
              </div>

              <div className="mt-3">
                <button onClick={() => checkout('devpass')} disabled={loading === 'checkout-devpass' || isPRO} className="btn rounded-full h-11 px-6 w-full shadow-soft disabled:opacity-60">
              ...
              <div className="mt-4">
                <button onClick={openPortal} disabled={loading === 'portal'} className="rounded-full h-11 px-7 inline-grid place-items-center border border-base bg-panel hover:bg-panel/80 disabled:opacity-60">
